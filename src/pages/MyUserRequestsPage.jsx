...
export default function MyUserRequestsPage() {
  ...
  const handleReject = async () => {
    ...
    try {
      await myOrganizationApi.rejectUserRequest(rejectDialog.request.id, rejectReason);
  ...
  return (
    <div className="space-y-6" data-testid="my-user-requests-page">
      <div>
        <h1 className="text-2xl md:text-3xl font-bold tracking-tight">User Requests</h1>
        <p className="text-muted-foreground mt-1">Manage user addition requests for your organization</p>
      </div>

      <Card className="border-border/50">
        <CardHeader>
          <CardTitle className="flex items-center gap-2"><UserPlus className="h-5 w-5" />Pending Requests ({pendingRequests.length})</CardTitle>
          <CardDescription>User requests awaiting your approval</CardDescription>
        </CardHeader>
        <CardContent>
          {loading ? (
            ...
            <div className="text-center py-8 text-muted-foreground">No pending user requests</div>
          ) : (
                    ...
                    <TableRow key={request.id}>
                      ...
                      <TableCell>
                        {request.approved_at || request.rejected_at
                          ? formatDistanceToNow(new Date(request.approved_at || request.rejected_at), { addSuffix: true })
                          : "N/A"}
                      </TableCell>
                    </TableRow>
                  ))}
      ...
      <Dialog open={approveDialog.open} onOpenChange={(open) => setApproveDialog({ open, request: null })}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Approve <strong>{approveDialog.request?.name}</strong> to join your organization.</DialogDescription>
          </DialogHeader>
      ...
      <Dialog open={rejectDialog.open} onOpenChange={(open) => setRejectDialog({ open, request: null })}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Reject <strong>{rejectDialog.request?.name}</strong>&apos;s request to join your organization.</DialogDescription>
          </DialogHeader>
          <div className="space-y-4 py-4">
            <div className="space-y-2">
